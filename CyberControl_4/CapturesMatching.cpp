...
void CapturesMatching::openMatrix(char *path){
	...
	if (!file){
		cout << "Error: оpening matrix perspective correction a failure" << endl;
		return;
	}
...
void CapturesMatching::correctivePerspective(IplImage *frame0){
	...
	srcQuad[1].x = width - 1;	srcQuad[1].y = 0;					//src Top right
	srcQuad[2].x = 0;			srcQuad[2].y = height - 1;			//src Bottom left
	srcQuad[3].x = width - 1;	srcQuad[3].y = height - 1;			//src Bot right
	//- - - - - - - - - - - - - -//
