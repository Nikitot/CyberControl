...
int* StereoPairCalibration::minCoordinats(CvPoint2D32f *corners, int board_w, int board_h){
	...
	for (int i = 0; i < board_w; i++){
		for (int j = 0; j < board_h; j++){
			...
			vals[2] = (int)MAX(vals[2], x);				//максимум по x
			vals[3] = (int)MAX(vals[3], y);				//максимум по y
...
bool StereoPairCalibration::findChess(IplImage *frame0, IplImage *frame1, char *name){
	...
	if (found1 && found2){
		...
		shiftImage(vals1[3] - vals2[3], 5, 7);										//сдвиг изображения по max(y)
		//shiftImage((vals1[2] - vals1[0]) - (vals2[2] - vals2[0]), 2, 6);			//сдвиг изображения x
		
		shiftImage(vals2[0] - vals1[0], 0, 4);										//сдвиг изображения по min(x)
		shiftImage(vals1[2] - vals2[2], 2, 6);										//сдвиг изображения по max(x)
