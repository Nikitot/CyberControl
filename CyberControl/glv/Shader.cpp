...
namespace glv {
    ...
    void Shader::setUniform( const char * struct_name, const char * variable, const glm::vec4 &vector ) {
        GLint location = getStructLocation( struct_name, variable );
        glUniform4f( location, vector.x, vector.y, vector.z, vector.w );
    }
    
    void Shader::setUniform( const char * uniform_name, const glm::vec4 &vector ) {
    ...
    void Shader::setUniform( const char * uniform_name, const glm::mat4 &matrix ) {
        if( uniforms.count( uniform_name ) == 0 ) {
            cerr << uniform_name << " was not generated previously" << endl;
            throw -1;
        }
